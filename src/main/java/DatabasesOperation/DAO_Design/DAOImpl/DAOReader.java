...
public class DAOReader {
    ...
    public List<ORM_Reader> read() {//返回所有的读者信息
        String sql = "select * from reader where id!=:id";
        Map<String, Integer> params = new HashMap<>();
        params.put("id", 0);
        List<ORM_Reader> list = nJDBC.query(sql, params, new BeanPropertyRowMapper<>(ORM_Reader.class));
     ...
     */
    public int deleteReader(ORM_Reader reader) {//刪除读者，限制只有它借的书都还了才能删除
    ...
    private int delete(ORM_Reader reader) {
    ...
    public void updateReaderHashBorrow(ORM_Reader reader) {
    ...
    public int updateReaderName(ORM_Reader reader,String name) {
        String sql = "update reader set name=:name  where name=:name1 and id=:id";
        Map<String, Object> params = new HashMap<>();
        params.put("name",name );
        params.put("name1", reader.getName());
        params.put("id",reader.getId());
        return nJDBC.update(sql, params);
    }
